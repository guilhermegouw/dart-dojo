...
Pero has a data plan with his cell phone provider that oﬀers him x megabytes of data 
per month. 
In addition, any data he doesn’t use in a given month carries over to the next month. 
For example: 
If x is 10 and Pero uses only 4MB in a given month, the remaining 6MB carry over to 
the next month (in which he’d now have 10 + 6 = 16MB available).

We’re given the number of megabytes of data that Pero
uses in each of the ﬁrst n months. 
...
int getAvailableData(int plan, int months, List<int> usage) {
  int available = plan;
